...
router.get("/park/edit/:id(\\d+)", asyncHandler(async (req, res) => {
    const parkId = parseInt(req.params.id, 10);
    const park = await db.Park.findByPk(parkId);
    res.render("park-edit", { title: "Edit Park", park, csrfToken: req.csrfToken() })
}));

router.post("/park/edit/:id(\\d+)", asyncHandler(async (req, res) => {
    const parkId = parseInt(req.params.id, 10);
    const update = await db.Park.findByPk(parkId);
